...
public class FileExplorerMethod {
	...
	public void printFileList(File dir) {
		...
		for(int i=0; i < subFile.length; i++) {
			...
			String fileSize = subFile[i].length()>=1000?"":"\t"; // console 창 출력위한
			System.out.println(booleanDir+"\t"+subFile[i].length()+"바이트\t\t"+fileSize+subFile[i].getName()+"\t\t");
		}
	...
	public String userCommand(Scanner sc) {
		...
		System.out.println("[..] 상위폴더로\t[dir이름] 해당 디렉터리 이동\t[rename A B] A파일을 B이름으로 변경\t[mkdir A] A라는 디렉토리 생성\t[그만] 시스템종료");
		System.out.print(">>");
	...
	public void fileRename(String oriFileName, String newFileName) {
		...
		File tempOriFile = new File(currentFilePath.getPath()+"/"+oriFileName);
		File tempNewFile = new File(currentFilePath.getPath()+"/"+newFileName);
		...
		if (!tempOriFile.exists()) { // 파일이 존재하지 않으면
			...
			tempOriFile.renameTo(tempNewFile);
