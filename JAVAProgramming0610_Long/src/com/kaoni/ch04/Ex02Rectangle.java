...
public class Ex02Rectangle {
   ...
   public static void printOverlap(Rectangle userRect, Rectangle comRect) {
      ...
      System.out.println(checklap?"[intersects 메소드] 영역이 겹칩니다.":"[intersects 메소드] 영역이 안 겹칩니다."+comRect.intersection(userRect));
      boolean checklap2 = intersectRectangle(userRect, comRect);
      System.out.println(checklap2?"[내가 만든 메소드] 영역이 겹칩니다.":"[내가 만든 메소드] 영역이 안 겹칩니다.");
   ...
   // 사각형 겹치는 지 확인하는 메소드
   public static boolean intersectRectangle(Rectangle userRect, Rectangle comRect) {
      boolean result = true; // true가 겹칩
      int userLeft = userRect.x;
      int userRight = userRect.x+userRect.width;
      int userBottom = userRect.y-userRect.height;
      int userTop = userRect.y;
      int comLeft = comRect.x;
      int comRight = comRect.x+comRect.width;
      int comBottom = comRect.y-comRect.height;
      int comTop = comRect.y;
      
      if(userLeft >= comRight) {
         result = false;
      } else if (userRight <= comLeft) {
         result = false;
      } else if (userBottom >= comTop) {
         result = false;
      } else if (userTop <= comBottom) {
         result = false;
      } 
      if(userRect.contains(comRect) && !userRect.equals(comRect)) {
    	  result = false;
      }
